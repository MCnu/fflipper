...
def findPathMatch(oldPath, searchDir="./"):
    ...
    if os.path.isfile(oldPath) == False:
        ...
        newPaths = filter(
            lambda s: re.match(os.path.sep.join((".*", basename)), s), dirContents
        )
        if len(newPaths) == 1:
...
class tierSet:
    ...
    def __init__(
        ...
        if file:
            tiers, media, relMedia, linkedFiles, relLinkedFiles = self.extractTiers(
                file
            )
    ...
    def extractTiers(self, file):
        ...
        for tierFound in root.findall("TIER"):
            if verbose:
                print(tierFound.attrib["TIER_ID"])
            annos = []
            for xx in tierFound:
        ...
        if len(linkedFiles) > 0:
            ...
            for linkedFile in linkedFiles:
                linkedFilePaths.append(
                    linkedFile.attrib["LINK_URL"][7:]
                )  # [7:] removes the file://
    ...
    def fixLinks(self, searchDir="./"):
        ...
        self.media = [
            os.path.abspath(findPathMatch(path, searchDir=searchDir))
            for path in self.media
        ]
        self.linkedFiles = [
            os.path.abspath(findPathMatch(path, searchDir=searchDir))
            for path in self.linkedFiles
        ]
        ...
        for tr in tiers:
            ...
            for anno in tr.annotations:
                if anno.begin >= begin and anno.end <= end:
                    newAnno = annotation(
                        anno.begin - tZero, anno.end - tZero, anno.value, anno.units
                    )
                    if verbose:
                        print(
                            "I added "
    ...
    def elanOut(
        tierObj,
        headFootFile=os.path.join(os.path.dirname(__file__), "elanSkeleton.eaf"),
        dest="./out.eaf",
    ):
        ...
        for mediaFile in mediaFiles:
            ...
            media.set(
                "RELATIVE_MEDIA_URL",
                "".join(["./", os.path.relpath(os.path.abspath(mediaFile), destDir)]),
            )
        ...
        if linkedFiles != [None]:
            for fl in linkedFiles:
                ...
                media.set(
                    "RELATIVE_LINK_URL",
                    "".join(["./", os.path.relpath(os.path.abspath(fl), destDir)]),
                )
        ...
        if tierObj.tiers != None:
            for tier in tierObj.tiers:
                ...
                for anno in tier.annotations:
                    if verbose:
                        print(
                            f"Working on time slot: '{str(tslt)}' and annotation: {str(anot)}"
                        )
                    tslt += 1
                    ...
                    alignable_annotation = ElementTree.SubElement(
                    ...
                    anno_value = ElementTree.SubElement(
                        alignable_annotation, "ANNOTATION_VALUE"
                    )
                    anno_value.text = anno.value

                    if verbose:
                        print(anno.value + ": " + str(anno.begin) + "-" + str(anno.end))
...
class timeSeries:
    ...
    def __init__(
        ...
        source=None,
        timeOrigin=None,
        sampleType="Continuos Rate",
        timeCol=0,
        tracks=None,
    ):
        if file:
            source, sampleType, tracks, timeCol, timeOrigin = self.extractTimeSeries(
                file
            )
            pathELAN = os.path.dirname(file)
        self.source = source
    ...
    def extractTimeSeries(self, file):
        ...
        rootLen = len(root)
        # get track source
        tracksource = root.findall("tracksource")[
            0
        ]  # maybe check how many there are. There should only ever be one.
    ...
    def timeSeriesOut(tsObj):
        ...
        tracksource.set(
            "source-url", "".join(["file://", os.path.abspath(tsObj.source)])
        )
